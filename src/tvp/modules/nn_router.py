...
from tvp.utils.utils import apply_dict_to_model
...
class NNRouter(AbstractRouter):
    def __init__(
        ...
        **kwargs,
        ...
        nn_heads = NNhead.build(self.encoder, cfg.eval_datasets, cfg.nn.module, cfg.nn.data.data_path, "cuda")
...
class NNhead(torch.nn.Module):
    def __init__(self, normalize: bool, buffer_size: int, weights: torch.Tensor, method: str = "set_distance"):
        super().__init__()
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        if self.method == "prototype":
            ...
            dists = torch.cdist(inputs, self.task_embeddings, p=2)  # shape: (B, num_tasks)
            return -dists
            ...
            flattened_emb = self.task_embeddings.view(num_tasks * buff_size, D)  # (num_tasks*buffer_size, D)
    ...
    def build(cls, encoder, task_names, cfg, data_location, device):
        ...
        with torch.no_grad():
            ...
            for task_name in tqdm(task_names):
                ...
                dataset = get_dataset(task_name, encoder.train_preprocess, location=data_location)
                batch = maybe_dictionarize(next(iter(dataset.train_loader)), cfg.x_key, cfg.y_key)
                x = batch[cfg.x_key]
        ...
        classification_head = cls(normalize=True, weights=nn_weights, buffer_size=x.shape[0])

...
from mass.pl_modules.pl_module import ImageClassifier


@st.cache(allow_output_mutation=True)
def get_model(checkpoint_path: Path):
    return load_model(module_class=ImageClassifier, checkpoint_path=checkpoint_path)
...
model: ImageClassifier = get_model(checkpoint_path=checkpoint_path)
model

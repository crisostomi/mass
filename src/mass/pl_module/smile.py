...
from typing import Any, Dict, List, Mapping, Optional, OrderedDict, Tuple, TypeAlias, Union  # noqa: F401
...
class SmileUpscalingAlgorithm(MultiHeadImageClassifier):
    ...
    def _upscale_linear_layer(
        ...
        pylogger.info(f"Upscaling layer {name} of type {type(module)} with shape {module.shape}")
    ...
    def _upscale_submodules(
        ...
        for name, module in tqdm(
            tuple(pretrained_model.named_parameters()),
            tqdm_desc,
            leave=False,
            dynamic_ncols=True,
        ):
            pylogger.info(f"Processing layer {name} of type {type(module)}")
            pylogger.info(f"Layer has shape {module.shape}")
            if len(module.shape) == 2:
                pylogger.info(f"Upscaling linear layer {name} of type {type(module)}")
                self.upscaled_layers.add(name)
                self._upscale_linear_layer(
                    pretrained_model=pretrained_model,
                    finetuned_models=finetuned_models,
                    name=name,
                )
            elif self.average_experts and len(module) == 1:
                pylogger.info(f"Averaging experts for leaf module {name} of type {type(module)}")
    ...
    def forward(self, inputs):
        ...
        for name, module in self.encoder.named_modules():
            ...
            if isinstance(module, SmileMoELinear) and hasattr(module, 'last_selected_experts'):
                collected_layers.add(name)
                votes.append(module.last_selected_experts)

        pylogger.info(f"Difference between upscaled layers and collected layers: {self.upscaled_layers - collected_layers}")

        if votes:
            votes = torch.stack(votes)
            majority_vote = torch.mode(votes, dim=0).values
            pylogger.info(f"Votes collected: {votes[:10]}, majority vote: {majority_vote}")
        else:
            majority_vote = torch.zeros(inputs.size(0), dtype=torch.long, device=inputs.device)

...
from mass.utils.utils import apply_dict_to_dict, apply_dict_to_model, compute_task_dict, sum_task_dict, print_memory
...
class TaskArithmeticMerger(TaskVectorBasedMerger):
    ...
    def merge_from_task_dicts(
        ...
        for dataset in task_dicts.keys():
            cumulative_dict = apply_dict_to_dict(task_dicts[dataset], cumulative_dict, coefficient=self.optimal_alpha[len(task_dicts)])

...
def run(cfg: omegaconf.DictConfig) -> str:
    ...
    benchmark_cfg = getattr(cfg, "benchmarks", None)
    ...
    for num_tasks in range(start_num_tasks, max_num_tasks + 1):
        ...
        cfg.eval_datasets = local_benchmarks
        ...
        finetuned_models = {
            ...
            for dataset in local_benchmarks
        }
        ...
        for dataset in local_benchmarks:
            task_dicts[dataset.name] = compute_task_dict(
                zeroshot_encoder.cpu().state_dict(),
                finetuned_models[dataset.name].state_dict(),
            )
        ...
        results = {}
        torch.cuda.empty_cache()
        pylogger.info(f"Starting evaluation for {num_tasks} tasks")
        for dataset_cfg in local_benchmarks:
            ...
            if cfg.eval_on_train:
                ...
                test_results = trainer.test(
                    model=model, dataloaders=dataset.train_loader
                )
        ...
        with open(results_path / f"{num_tasks}.json", "w+") as f:
...
@hydra.main(config_path=str(PROJECT_ROOT / "conf"), config_name="moerging_scaling.yaml")

...
def run(cfg: DictConfig) -> str:
    ...
    total_benchmarks = len(benchmark_datasets)
    ...
    for num_tasks in range(start_num_tasks, max_num_tasks + 1):
        local_benchmarks = benchmark_datasets[:num_tasks]
        pylogger.info(f"Starting static merging scaling eval for {num_tasks} tasks")
...
@hydra.main(config_path=str(PROJECT_ROOT / "conf"), config_name="merging_scaling.yaml")
